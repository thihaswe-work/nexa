...
import { useRegistrationAnalytics } from '@/lib/hooks/use-admin';
...
export default function RegistrationsPage() {
  ...
  const { data, isLoading } = useRegistrationAnalytics({
    from: range.from.toISOString(),
    to: range.to.toISOString(),
  });

  const metrics = [
    { title: 'Total Users', value: data?.total, icon: Users, color: 'text-blue-600', bg: 'bg-blue-100 dark:bg-blue-900/20' },
    { title: 'This Week', value: data?.thisWeek, icon: TrendingUp, color: 'text-emerald-600', bg: 'bg-emerald-100 dark:bg-emerald-900/20' },
    { title: 'This Month', value: data?.thisMonth, icon: CalendarDays, color: 'text-violet-600', bg: 'bg-violet-100 dark:bg-violet-900/20' },
  ];
    ...
    <div className="space-y-6 animate-in">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold">Daily Registrations</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
